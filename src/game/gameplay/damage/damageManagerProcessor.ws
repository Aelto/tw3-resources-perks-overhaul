...
class W3DamageManagerProcessor extends CObject 
{
	...
	private function ProcessActionDamage() : bool
	{
		...
		RPO_damageModifier(action, this.playerAttacker, this.attackAction); // RPO
	...
	private function ProcessActionBuffs() : bool
	{
		...
		// RPO - BEGIN & END
		if(!action.victim.IsAlive() || action.WasDodged() || (attackAction && attackAction.IsActionMelee() && !attackAction.ApplyBuffsIfParried() && (attackAction.CanBeParried() || RPO_canParryAnything()) && attackAction.IsParried()) )
			return true;
